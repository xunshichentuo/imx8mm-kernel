...
static s8 gtp_i2c_test_after_rst(struct i2c_client *client)
{
    ...
    if (res < 0)
    {
        GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d", (s32)gtp_int_gpio, res);
        res = -ENODEV;
    }

    res = GTP_GPIO_REQUEST(gtp_rst_gpio, "GTP RST PORT");
    if (res < 0)
    {
        GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d",(s32)gtp_rst_gpio,res);
        res = -ENODEV;
    }
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
   ...
   // GTP_INFO("GTP Driver Built@%s, %s", __TIME__, __DATE__);
    ...
    ret = gtp_i2c_test_after_rst(client);
    if (ret < 0)
    {
