...
#define RX8010_RAM_USERVAL  0x01
...
static int rx8010_get_time(struct device *dev, struct rtc_time *dt)
{
	...
	u8 test[1];
	u8 ram[1];
	err = i2c_smbus_read_i2c_block_data(rx8010->client, RX8010_REG_USER0, 1, test);
	if (err != 1)
		return err < 0 ? err : -EIO;

	err = i2c_smbus_read_i2c_block_data(rx8010->client, RX8010_REG_USER1, 1, ram);
	if (err != 1)
		return err < 0 ? err : -EIO;
...
static int rx8010_set_time(struct device *dev, struct rtc_time *dt)
{
	...
	ret = i2c_smbus_write_byte_data(rx8010->client, RX8010_REG_USER0, RX8010_RAM_USERVAL);
	if (ret < 0)
		return ret;

	ret = i2c_smbus_write_byte_data(rx8010->client, RX8010_REG_USER1, RX8010_RAM_USERVAL);
	if (ret < 0)
		return ret;

	ret = i2c_smbus_read_i2c_block_data(rx8010->client, RX8010_REG_USER1, 1, ram);
	if (ret != 1)
...
static int rx8010_init_client(struct i2c_client *client)
{
	...
	if (ctrl[0] & RX8010_FLAG_VLF){
		...
		err = i2c_smbus_read_i2c_block_data(rx8010->client, RX8010_REG_USER0, 1, ram);
		if (err != 1)
			return err < 0 ? err : -EIO;

		err = i2c_smbus_write_byte_data(client, RX8010_REG_USER1, ram[0]);
		if (err < 0)
			return err;

		if (ram[0] & RX8010_RAM_USERVAL)
			need_clear = 0;
		else
			need_clear = 1;
	}
